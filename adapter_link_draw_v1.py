...
def angle_in_angle_out(p0x, p0y, s, L1, L2, L3, th1):
    p1x = L1 * np.sin(th1)
    p1y = - L1 * np.cos(th1)
    ...
    return(th3, Lc, alpha, p1x, p1y, p2x, p2y, p3x, p3y)
...
def main():
    ...
    # theta 1 in radians
    angle_max_deg = 60
    th1 = np.linspace(0, np.radians(angle_max_deg))
    #print("theta 1 is %f\n" % th1)

    angle_out = 0 * th1
    L1_array = (1/2) * np.array([1, 1.1, 1.2, 1.25]) * s
    ...
    for L1 in L1_array:
        ...
        for i, angle_in in enumerate(th1):
            foo = angle_in_angle_out(p0x, p0y, s, L1, L2, L3, angle_in)
            angle_out[i] = foo[0]
            print(foo)
        ax.plot(np.degrees(th1), np.degrees(angle_out - th1))
    ax.legend(["L1 = 1", "L1 = 1.1", "L1 = 1.2", "L1 = 1.25"])
    ax.plot([0, angle_max_deg], [0, 0], 'k', linewidth=guide_width, alpha=guide_alpha)
    plt.xlabel('angle input (degrees)')
    plt.ylabel('angle output - angle input (degrees)')
